...
def make_batches(lines, args, task, max_positions, encode_fn, truncate_size):
    ...
    tokens = [
        task.source_dictionary.encode_line(
            encode_fn(src_str), add_if_not_exist=False, max_tokens = truncate_size
        ).long()
...
def main(args):
    ...
    for inputs in buffered_read(args.input, args.buffer_size):
        output_count = 0
        results = []
        for batch in make_batches(inputs, args, task, max_positions, encode_fn, args.truncate_size):
        ...
        for id_, src_tokens, hypos, info in sorted(results, key=lambda x: x[0]):
            ...
            for hypo in hypos[:min(len(hypos), args.nbest)]:
                ...
                positional_score = sum(hypo['positional_scores'].div_(math.log(2)).tolist())


                if args.nbest == 1:
                    fout.write(detok_hypo_str.replace('\n', ' ')+'\n')
                else:
                    fout.write('{} ||| {} ||| F0= {:.6f} ||| {:.6f}\n'.format(id, detok_hypo_str.replace('\n', ' '), positional_score, score))
                output_count += 1
        ...
        if args.nbest == 1:
            assert output_count == len(inputs), (len(inputs), output_count)
        fout.flush()
        
        # update running id counter
        start_id += len(inputs)

        logger.info('Translated: {}'.format(start_id))
